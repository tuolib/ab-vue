...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    const isActiveItem = (id: Numeric) =>
      Array.isArray(props.activeId) ? props.activeId.includes(id) : props.activeId === id;
    ...
    const renderSubItem = (item: TreeSelectChild) => {
      ...
      return (
        ...
        >
          {item.text}
          {isActiveItem(item.id) && <span class={bem('selected')} />}
        </div>
      );
    };
    ...
    const renderSidebar = () => {
      ...
      return (
        <Sidebar class={bem('nav')} modelValue={props.mainActiveIndex} onChange={onSidebarChange}>
          {Items}
        </Sidebar>
      );
    };
