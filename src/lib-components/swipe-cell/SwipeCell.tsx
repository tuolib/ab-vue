import { ref, Ref, reactive, computed, defineComponent } from 'vue';
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-shadow
    const renderSideContent = (side: SwipeCellSide, ref: Ref<HTMLElement | undefined>) => {
      const contentSlot = slots[side];
      if (contentSlot) {
        return (
          <div ref={ref} class={bem(side)} onClick={getClickHandler(side, true)}>
            {contentSlot()}
          </div>
        );
      }
