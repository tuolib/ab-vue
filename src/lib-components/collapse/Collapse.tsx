...
import { truthProp, createNamespace, BORDER_TOP_BOTTOM, type Numeric } from '../utils';
  ...
  setup(props, { emit, slots }) {
    ...
    const toggle = (name: Numeric, expanded: boolean) => {
      ...
      if (accordion) {
        ...
        updateName((modelValue as Numeric[]).filter(activeName => activeName !== name));
      }
    };
    ...
    const isExpanded = (name: Numeric) => {
      ...
      return accordion ? modelValue === name : (modelValue as Numeric[]).includes(name);
    };

    linkChildren({ toggle, isExpanded });

    return () => (
      <div class={[bem(), { [BORDER_TOP_BOTTOM]: props.border }]}>{slots.default?.()}</div>
    );
  },
});
