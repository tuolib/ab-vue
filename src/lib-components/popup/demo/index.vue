...
<template>
  ...
  <div card :title="t('roundCorner')">
    <van-cell :title="t('roundCorner')" is-link @click="showRoundCorner = true" />
    <van-popup v-model:show="showRoundCorner" round position="bottom" :style="{ height: '30%' }" />
  </div>

  <div card :title="t('teleport')">
    <van-cell :title="t('teleport')" is-link @click="showGetContainer = true" />
    <van-popup v-model:show="showGetContainer" teleport="body" :style="{ padding: '30px 50px' }" />
  </div>
</template>
