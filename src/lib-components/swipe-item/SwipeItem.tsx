  ...
  setup(props, { slots }) {
    ...
    if (!parent) {
      if (process.env.NODE_ENV !== 'production') {
        console.error('[Abv] <SwipeItem> must be a child component of <Swipe>.');
      }
      return;
    }
    ...
    const style = computed(() => {
      ...
      if (parent.size.value) {
        styles[vertical ? 'height' : 'width'] = `${parent.size.value}px`;
      }

      if (state.offset) {
        styles.transform = `translate${vertical ? 'Y' : 'X'}(${state.offset}px)`;
      }
