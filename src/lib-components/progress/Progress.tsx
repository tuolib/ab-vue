...
import { addUnit, truthProp, numericProp, createNamespace, type Numeric } from '../utils';
  ...
  setup(props) {
    ...
    const renderPivot = () => {
      ...
      if (props.showPivot && text) {
        ...
        return (
          <span style={style} class={bem('pivot', { inactive: props.inactive })}>
            {text}
          </span>
        );
      }
    ...
    return () => {
      ...
      return (
        <div class={bem()} style={rootStyle}>
          <span class={bem('portion', { inactive: props.inactive })} style={portionStyle} />
          {renderPivot()}
        </div>
      );
    };
