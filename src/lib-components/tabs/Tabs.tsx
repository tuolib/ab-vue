...
import type { Numeric, Interceptor, ComponentInstance } from '../utils';
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const getTabName = (tab: ComponentInstance, index: number): Numeric => tab.name ?? index;
    ...
    const onClickTab = (item: ComponentInstance, index: number, event: MouseEvent) => {
      ...
      return (
        ...
        >
          <div
            ref={navRef}
            role="tablist"
            class={bem('nav', [type, { shrink: props.shrink, complete: scrollable.value }])}
    ...
    // eslint-disable-next-line @typescript-eslint/no-shadow
    const onRendered = (name: Numeric, title?: string) => emit('rendered', name, title);
    ...
    return () => (
      <div ref={root} class={bem([props.type])}>
        {props.sticky ? (
          <Sticky container={root.value} offsetTop={offsetTopPx.value} onScroll={onStickyScroll}>
            {renderHeader()}
            {slots['nav-bottom']?.()}
          </Sticky>
        ) : (
