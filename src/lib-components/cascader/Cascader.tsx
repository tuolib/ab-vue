import { ref, watch, nextTick, defineComponent, type PropType, type ExtractPropTypes } from 'vue';
import {
  extend,
  truthProp,
  numericProp,
...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    const onClickTab = ({ name, title }: TabsClickTabEventParams) => emit('clickTab', name, title);

    const renderHeader = () =>
      props.showHeader ? (
        <div class={bem('header')}>
          <h2 class={bem('title')}>{slots.title ? slots.title() : props.title}</h2>
          {props.closeable ? (
            <span
              // @ts-ignore
    ...
    const renderOption = (
      ...
      return (
        ...
        >
          {Text}
          {selected ? <span class={bem('selected-icon')} /> : null}
        </li>
      );
    };
